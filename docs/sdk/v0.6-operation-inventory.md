...
2. `installed_tools.list` / `installed_tools.connection_readiness`
Reason: installed-tool read coverage complements the buyer-side install flow and stays in the same thin owner-operation wrapper family.
3. `installed_tools.execution.get` / `installed_tools.receipts.get`
...
4. `wallet.balance.get` / `wallet.accounts.list`
Reason: useful for owner dashboards, but the wallet namespace is broader and more likely to overlap with ongoing settlement and payout work than proposals or works.

Historical note: `partner.*` and `ads.*` read wrappers were added in PR-S2e. The remaining shortlist above keeps the earlier rationale but drops the now-covered partner / ads items.
