...
- `not exposed`: the operation exists in the registry, but the public SDK intentionally does not surface it yet. Today this bucket is dominated by the remaining `account.*` namespaces after PR-Qa plus `admin.*` operations that remain out of scope for public SDK consumers.
