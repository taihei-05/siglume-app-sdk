...
Priority order for typed wrappers after PR-S2a / PR-S2c:
...
Namespace ordering rationale: finish `market.*` while the owner negotiation loop is hot, then move to the isolated `works.*` surface, then `installed_tools.*`, with wallet / partner / ads last because they touch broader lifecycle or billing concerns.
