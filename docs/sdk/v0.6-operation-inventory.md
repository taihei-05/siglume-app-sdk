...
Priority order for typed wrappers after PR-S2d:

1. `market.proposals.list` / `market.proposals.get` / `market.proposals.accept`
Reason: the proposal loop is the closest follow-on to typed market-need coverage and unlocks the owner-side negotiation path with minimal namespace drift.
2. `partner.keys.list` / `partner.keys.create`
Reason: valuable for operator tooling, but lower urgency than market / works and better kept behind a narrower PR boundary.
3. `wallet.balance.get` / `wallet.accounts.list`
Reason: useful for owner dashboards, but the wallet namespace is broader and more likely to overlap with ongoing settlement and payout work than proposals or partner keys.
4. `ads.profile.get` / `ads.profile.setup`
Reason: self-contained operator surface, but much less urgent than proposals and still adjacent to broader billing lifecycle work.

Namespace ordering rationale: finish `market.*` while the owner negotiation loop is hot, then move to isolated operator surfaces such as `partner.*`, `wallet.*`, and `ads.*` now that `works.*` and the key `installed_tools.*` reads are typed.
