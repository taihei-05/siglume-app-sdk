...
optional `input_form_spec.json`, runs CLI preflight by default, then calls the
same `auto-register` route used by SDK / automation clients. The server-side
publish gate includes runtime checks, contract checks, pricing / payout rules,
and a mandatory fail-closed LLM legal review for law compliance plus
